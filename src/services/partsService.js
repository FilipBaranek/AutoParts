...
exports.carParts = async (partData) => {
    ...
    {
        parts = await Parts.getParts(partData.subcat, decodeEngine(partData.engine));
    }
...
exports.partBrands = async (partData) => {
    ...
    if (partData.search === "car")
    {
        brands = await Parts.findBrands(partData.subcat, decodeEngine(partData.engine));
    }
    else if (partData.search === "code")
    {
...
exports.sortParts = async (partData) => {
    ...
    filters.builtParams.unshift(partData.partName, decodeEngine(partData.engineName));
