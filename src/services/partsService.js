...
exports.carParts = async (partData) => {
    ...
    {
        parts = await Parts.getParts(decodeURL(partData.subcat), decodeURL(partData.engine));
    }
...
exports.partBrands = async (partData) => {
    ...
    if (partData.search === "car")
    {
        brands = await Parts.findBrands(partData.subcat, decodeURL(partData.engine));
    }
    else if (partData.search === "code")
    {
...
exports.sortParts = async (partData) => {
    ...
    filters.builtParams.unshift(partData.partName, decodeURL(partData.engineName));
